...
ax.set_title('🚀 THE GRAND TIMELINE: Company Arrival Patterns & Strategic Insights\nBubble Size = CTC | Color = Tier', 
...
# ============================================================================
# PLOT 4: TIER MIGRATION - The Strategic Shift
# ============================================================================
print("\nCreating Plot 4: Tier Migration...")

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(22, 10))
fig.patch.set_facecolor('white')

# Left: Stacked area over time
df_valid['Week'] = df_valid['Date_OA'].dt.to_period('W').dt.to_timestamp()
weekly_tier = df_valid.groupby(['Week', 'Tier']).size().unstack(fill_value=0)

ax1.fill_between(weekly_tier.index, 0, weekly_tier['Tier 1'], 
                 color='#00b894', alpha=0.7, label='Tier 1')
ax1.fill_between(weekly_tier.index, weekly_tier['Tier 1'], 
                 weekly_tier['Tier 1'] + weekly_tier['Tier 2'],
                 color='#fdcb6e', alpha=0.7, label='Tier 2')
ax1.fill_between(weekly_tier.index, 
                 weekly_tier['Tier 1'] + weekly_tier['Tier 2'],
                 weekly_tier['Tier 1'] + weekly_tier['Tier 2'] + weekly_tier['Tier 3'],
                 color='#e17055', alpha=0.7, label='Tier 3')

ax1.set_xlabel('Week', fontsize=14, fontweight='bold')
ax1.set_ylabel('Number of Companies', fontsize=14, fontweight='bold')
ax1.set_title('📈 Weekly Tier Distribution (Stacked Area)', fontsize=16, fontweight='bold')
ax1.legend(loc='upper left', fontsize=12, framealpha=0.95)
ax1.grid(True, alpha=0.3)

# Add insight
early_tier1_pct = (weekly_tier['Tier 1'].iloc[:4].sum() / weekly_tier.iloc[:4].sum().sum() * 100)
ax1.text(0.5, 0.95, f'💡 First 4 weeks: {early_tier1_pct:.0f}% Tier 1!\nPremium companies arrive EARLY',
        transform=ax1.transAxes, ha='center', va='top', fontsize=11, fontweight='bold',
        bbox=dict(boxstyle='round,pad=0.8', facecolor='yellow', alpha=0.8, edgecolor='orange', linewidth=2))

# Right: Percentage composition
weekly_pct = weekly_tier.div(weekly_tier.sum(axis=1), axis=0) * 100

ax2.stackplot(weekly_pct.index, 
             weekly_pct['Tier 1'], 
             weekly_pct['Tier 2'],
             weekly_pct['Tier 3'],
...
ax2.set_xlabel('Week', fontsize=14, fontweight='bold')
ax2.set_ylabel('Percentage (%)', fontsize=14, fontweight='bold')
ax2.set_title('📊 Weekly Tier Composition (%)', fontsize=16, fontweight='bold')
...
for start, end, label, color in phases:
    ...
    ax2.axvline(mid, color=color, linestyle='--', linewidth=2, alpha=0.6)

plt.suptitle('🎯 TIER MIGRATION: Strategic Shift in Company Quality Over Time', 
            fontsize=20, fontweight='bold', y=0.98)
plt.tight_layout()
plt.savefig('PLOT4_Tier_Migration.png', dpi=300, bbox_inches='tight', facecolor='white')
print("✓ Saved: PLOT4_Tier_Migration.png")
plt.close()
